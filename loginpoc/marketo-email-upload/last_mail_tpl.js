...
kinds.forEach(function(kind,kindIndex){
	mkdirp.sync("./last_email/"+kind+"/");	
	var templateFile = fs.readFileSync('./last_email/live_template.json', "utf8");
	if(pageIds[kindIndex]==="du" || pageIds[kindIndex]==="dnu"){
		templateFile = fs.readFileSync('./last_email/template.json', "utf8");
	}	
	languages.forEach(function(language) {
		var string = fs.readFileSync('./last_email/'+language+".txt", "utf8");
		...
		var targetFilePath="./last_email/"+kind+"/"+language+".json";
...
kinds.forEach(function(kind, kindIndex){
	var templateFile = fs.readFileSync('./last_email/template.html', "utf8");
	if(pageIds[kindIndex]==="du" || pageIds[kindIndex]==="dnu"){
		templateFile = fs.readFileSync('./last_email/demo_template.html', "utf8");
	}
	languages.forEach(function(language){
		var dataFilePath="./last_email/"+kind+"/"+language+".json";
		...
		var targetFilePath=__dirname + '/last_email/final-emails/'+language+'/'+templateData.targetFileName;		
		mkdirp.sync(__dirname + '/last_email/final-emails/'+language+'/');
